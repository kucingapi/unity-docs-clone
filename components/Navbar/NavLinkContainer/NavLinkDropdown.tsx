  ...
  return (
    <div>
      <div className="group inline-block relative">
        ...
        <ul className="absolute -translate-x-1/3 hidden group-hover:block border-gray-600 border shadow-xl">
          <li className="">
            {dropdowns.map(({ title, link }, index) => {
              console.log({title,link});
              return(
                <NavLink
                  className="text-left bg-black w-full py-2 px-4 pr-10 block whitespace-no-wrap text-sm"
                  title={title}
                  link='link'
                />
              )
